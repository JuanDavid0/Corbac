...
function filtrarValor() {
    ...
    xmlhttp.open("GET", rutaOculta+"contenido/ajax/controladorGaleria.php?accion=filtrar&texto="+texto, true);
...
function eliminarValor() {
    ...
    if (id !== '') {
        ...
        xmlhttp.onreadystatechange = function () {
            if ((this.readyState === 4 && this.status === 200)) {                                                    
                window.location = rutaOculta+"nosotrosadmin/"+this.responseText;
            }
        };
        xmlhttp.open("GET", rutaOculta+"contenido/ajax/ajaxNosotros.php?accion=eliminar&id="+id, true);
        xmlhttp.send();
        }
}
