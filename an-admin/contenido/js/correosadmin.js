...
function filtrarValor() {
    ...
    xmlhttp.open("GET", rutaOculta+"contenido/ajax/ajaxCorreo.php?accion=filtrar&texto="+texto, true);
...
function eliminarValor() {       
    ...
    if (id !== '') {
        ...
        xmlhttp.onreadystatechange = function () {
            if ((this.readyState === 4 && this.status === 200)) {                                                    
                window.location = rutaOculta+"correosadmin/"+this.responseText;
            }
        };
        xmlhttp.open("GET", rutaOculta+"contenido/ajax/ajaxCorreo.php?accion=eliminar&id="+id, true);
        xmlhttp.send();
        }
}
