...
function filtrarValor() {
    ...
    xmlhttp.open("GET", rutaOculta + "contenido/ajax/ajaxNoticia.php?accion=filtrar&texto=" + texto, true);
...
function eliminarValor() {
    ...
    if (id !== '') {
        ...
        xmlhttp.onreadystatechange = function () {
            if ((this.readyState === 4 && this.status === 200)) {
                window.location = rutaOculta + "noticiasadmin/" + this.responseText;
            }
        };
        xmlhttp.open("GET", rutaOculta + "contenido/ajax/ajaxNoticia.php?accion=eliminar&id=" + id, true);
        xmlhttp.send();
    }
}
